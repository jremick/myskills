...
Open `http://localhost:3000` to browse approved skills, inspect release export guidance, and sign in with the seeded owner account. The browser UI supports MFA challenge completion when the account requires it, and owner/admin sessions can open the admin console for registration mode, user status actions, provider metadata/mapping management, and audit review.
