...
test("MCP API URLs reject embedded credentials and query parameters", () => {
  assert.throws(
    () => createRegistryApiClient({ apiBaseUrl: "https://user:secret@example.test/api" }),
    /without credentials or query parameters/,
  );
  assert.throws(
    () => createRegistryApiClient({ apiBaseUrl: "https://example.test/api?token=secret" }),
    /without credentials or query parameters/,
  );
});
...
test("install instructions never fetch or expose bundle package contents", async () => {
  ...
  assert.equal(text.includes("apiBundleEndpoint"), false);
  assert.equal(text.includes("/v1/skills/release-notes-helper/releases/0.1.0/bundle"), false);
...
test("architecture tools forward auth and expose only safe owner projections", async () => {
      ...
      if (url.endsWith("/v1/architecture-patterns")) {
        return jsonResponse(200, {
          patterns: [{
            ...
            description: "Nested routers and skill leaves. Public docs: /architecture/overview; local C:\\Users\\jarel\\private\\architecture.json and \\\\server\\share\\architecture.json.",
      ...
      if (url.endsWith("/v1/architectures")) {
        return jsonResponse(200, {
          architectures: [{
            ...
            access: {
              allowedOrganizationIds: ["org-z", "org-a", "org-a"],
  ...
  assert.equal(combined.includes("user-secret"), false);
  assert.match(combined, /allowedOrganizationIds/);
  ...
  assert.equal(combined.includes("/architecture/overview"), true);
  assert.equal(combined.includes("C:\\Users\\jarel"), false);
  assert.equal(combined.includes("\\\\server\\share"), false);
  assert.equal(combined.includes("jarel"), false);
  ...
  const safeArchitectureList = architectures.structuredContent?.architectures as Array<Record<string, unknown>>;
  assert.deepEqual(safeArchitectureList[0]?.allowedOrganizationIds, ["org-a", "org-z"]);
...
test("architecture projection rejects an unsafe organization context before preview", async () => {
  const calls: string[] = [];
  const client = createRegistryApiClient({
    token: "aiss_test_secret",
    fetchImpl: async (url) => {
      ...
      if (url.endsWith("/v1/architectures/arch-1")) {
        return jsonResponse(200, { architecture: { id: "arch-1" } });
      }
      return jsonResponse(500, {});
    },
  });

  const result = await createAiSkillsMcpHandlers(client).getArchitectureProjection({
    id: "arch-1",
...
test("architecture tools accept architectures:read without skills:read", async () => {
  const calls: string[] = [];
  const client = createRegistryApiClient({
    token: "aiss_test_secret",
    fetchImpl: async (url) => {
      ...
      assert.equal(url, "http://localhost:3001/v1/architectures");
      return jsonResponse(200, { architectures: [{ id: "arch-1", name: "Personal skills" }] });
    },
  });

  const result = await createAiSkillsMcpHandlers(client).listArchitectures();

  assert.equal(result.isError, undefined);
  assert.deepEqual(result.structuredContent, {
    architectures: [{ id: "arch-1", name: "Personal skills" }],
    count: 1,
  });
  assert.deepEqual(calls, [
    "http://localhost:3001/v1/mcp/session",
...
test("MCP tools fail closed for malformed or interactive session credentials", async () => {
  const client = createRegistryApiClient({
    token: "aiss_test_secret",
    fetchImpl: async () => jsonResponse(200, {
      credential: {
        kind: "session",
        tokenId: "session-1",
        scopes: ["skills:read", "architectures:read"],
      },
    }),
  });
...
test("architecture not-found errors stay architecture-specific and sanitized", async () => {
  const client = createRegistryApiClient({
    token: "aiss_test_secret",
    fetchImpl: async (url) => {
      if (url.endsWith("/v1/mcp/session")) {
      ...
      return jsonResponse(404, {
        error: {
          ...
          message: "private architecture /Users/jarel/secret/spec.json",
        },
      });
    },
  });

  const result = await createAiSkillsMcpHandlers(client).getArchitectureProjection({ id: "arch-1" });
  const text = JSON.stringify(result);

  assert.equal(result.isError, true);
  assert.match(text, /Architecture not found/);
  assert.equal(text.includes("/Users/jarel"), false);
  assert.equal(text.includes("private architecture"), false);
});
