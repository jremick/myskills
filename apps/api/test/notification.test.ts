...
test("SMTP auth notification sink formats verification, reset, invitation, and email-change messages", async () => {
  ...
  assert.equal(sent[3].to, "new@example.com");
  assert.equal(sent[3].subject, "Confirm your new MySkills email");
  assert.match(String(sent[3].text), /https:\/\/skills\.example\/auth\/change-email#token=change-token/);
...
test("Resend auth notification sink formats verification, reset, invitation, and email-change messages", async () => {
  const sent: Array<Record<string, unknown>> = [];
