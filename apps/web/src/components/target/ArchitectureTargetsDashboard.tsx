...
const SUPPORTED_ADAPTERS = [
  ...
  { kind: "codex-companion", version: "1", contractVersion: 2 as const, label: "Codex · update companion" },
] as const;
...
function RegisterTargetCard({ client, session, onRegistered }: { client: RegistryClient; session: TargetSession; onRegistered: (target: ArchitectureTargetRecord) => void }) {
  ...
  useEffect(() => {
    setCapabilities((current) => selectedAdapter.contractVersion === 2
      ? { ...current, "inventory.read": true, apply: true, rollback: true, "sync.write": true }
      : { "inventory.read": true, "health.read": current["health.read"] !== false, "plan.read": current["plan.read"] !== false });
  }, [selectedAdapter.contractVersion]);
  ...
  return <Card className="control-plane-card" aria-label="Register connected target"><CardHeader className="control-plane-card-heading"><div className="control-plane-card-icon"><Plus size={17} aria-hidden="true" /></div><div><CardTitle>Register connected target</CardTitle><CardDescription>Choose a server-authorized owner, then bind either a read-only observer or an update companion to the current architecture revision, profile, and logical environment.</CardDescription></div></CardHeader><CardContent><form className="control-plane-form target-register-form" onSubmit={(event) => void submit(event)}>
    ...
    <fieldset className="target-capability-fieldset"><legend>Capabilities</legend>{READ_CAPABILITY_OPTIONS.map((option) => <label className="control-plane-checkbox" key={option.key}><input checked={capabilities[option.key] === true} disabled={state === "saving"} type="checkbox" onChange={(event) => toggleCapability(option.key, event.target.checked)} /><span><strong>{option.label}</strong><small>{option.description}</small></span></label>)}{selectedAdapter.contractVersion === 2 && <><label className="control-plane-checkbox"><input checked={capabilities.apply === true} disabled type="checkbox" /><span><strong>Apply</strong><small>Install and update an exact reviewed artifact.</small></span></label><label className="control-plane-checkbox"><input checked={capabilities.rollback === true} disabled type="checkbox" /><span><strong>Rollback</strong><small>Restore a verified local snapshot.</small></span></label><p className="control-plane-muted">Write capabilities are paired with sync.write and are enforced by contract v2, consent, policy, scoped tokens, leases, and receipts.</p></>}</fieldset>
