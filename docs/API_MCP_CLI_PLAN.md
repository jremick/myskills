...
The current web slice consumes backend API decisions for login, current-user refresh, public skill search, skill detail, release metadata, export guidance, and owner/admin console operations. It forwards the active bearer token when present, but it must not fetch bundle payloads during metadata browsing or reimplement authorization policy in client code.
