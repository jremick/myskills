...
A first target policy under an existing organization ceiling starts without extra restrictions: prereleases allowed, all change kinds allowed, no pins, and manual mode without a maintenance window. Without an organization ceiling, it preserves the stable-channel default until the user explicitly enables prereleases. The organization ceiling is shown read-only and remains independently enforced. Editing an existing target preserves its own settings. Organization rules are not copied into target revisions, so later organization pin or window changes do not leave a stale copy on the target.
