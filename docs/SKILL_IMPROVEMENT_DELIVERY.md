...
- One bounded real Claude Code adapter probe against public fixtures. Fixture results and real model results are recorded separately.
...
| Local CLI workflows | 14 journeys passed | `node --import tsx --test apps/cli/test/skill-improvement-journey.test.ts` |
| Core policy boundaries | 22 checks passed, including exhaustive reviewer role sets | `node --import tsx --test packages/core/test/improvement-policy-boundaries.test.ts` |
| API workflows | Six HTTP journeys passed | `node --import tsx --test apps/api/test/skill-improvement-journey.test.ts`; sanitized JSON journals in private `myskills-journey-evidence-*` subdirectories beneath `MYSKILLS_JOURNEY_EVIDENCE_DIR` or the OS temporary directory |
...
- The user approved Claude Code as the first supported runner on 26 September 2026. A real Claude Code 2.1.283 public-fixture probe returned the exact `claude-opus-5-5` model, only the `StructuredOutput` tool, no MCP servers or loaded skills, no planted CLAUDE.md/AGENTS.md markers, and no hook execution. The full native adapter journey then passed seven calls in 36.5 seconds: the exact model was observed on every call, the source stayed unchanged, no planted hook ran, and the sealed candidate exported successfully. Development, holdout and protected text cases improved from 0/3 baseline passes to 3/3 candidate passes. This is a public synthetic workflow check, not a general model-quality benchmark. No credentials were copied and no new authentication path was created.
...
The CLI rejects older versions, legacy Codex plans, model aliases, unsafe init/tool events, unknown event types, malformed output, model drift and error results. Fourteen executable-fixture journeys cover the launch flag/environment allowlist, 30 stream deviations, descendant cleanup, cancellation, timeout, byte bounds, UTF-8 stream splits, registry model receipts and exact candidate export. Tests were written before their implementation changes.

The real run exposed a `thinking_tokens` progress event that the first fixture omitted. The parser now accepts only its numeric counters after initialization; those estimates never become token accounting. A one-turn limit bounds each invocation. Native host transport retries remain outside the coordinator's count. Safe mode excludes user/project customizations; organization-managed policy may still apply. The adapter is not an OS sandbox, and event rejection cannot undo a host-managed hook. See [Claude Code CLI reference](https://code.claude.com/docs/en/cli-reference).

Public fixture evidence binds source `e96c28531c2a773f5d6b6a7cd0a2361e4e250aed8a9b0dda5a25f7099c7826c3` and candidate `f6a0dff81bf429f0e8c3bafff64c198a5d18570871472af6bb1ffe55d41f5d34`, with `text-contract` measurement and `local-report` provenance. A public canary outside the job, referenced with `@absolute-path` in the reviewer, did not appear in findings or candidate content. The coordinator escapes `@` in the serialized JSON without changing the parsed task, and checks the exact CLI version on every call. Native Windows execution is blocked. Real Claude execution was verified on macOS; Linux has executable-fixture coverage through Windows-hosted containers. WSL is unverified. No raw provider event stream is retained by the product.

The final read-only Opus review confirmed that the file-expansion and version-drift findings are closed, with no new high-severity or blocking finding in those changes. Assistant transcript metadata again confirmed `claude-opus-5-5`; delegation metadata recorded `xhigh` effort on the personal subscription route. The native canary run completed after that review and confirmed the result stated above. The reviewed launcher limitation remains: initialization is checked after dispatch, and a launcher can point to different same-version bytes. Prefer a native binary pin when that distinction matters.

The final adapter source passed all 14 CLI journeys and six full-stack journeys. The preceding full repository check passed on Windows; the exact committed adapter candidate still requires the canonical release gate and fresh GitHub checks.
...
The first CI scan reported five findings. The URL detector now scans once instead of using an unanchored repeating expression. API test journals use private, randomly allocated temporary directories and exclusive file creation. The two audit leak checks compare bytes rather than resembling URL validation. The bounded URL boundary checks were written before the detector change. CodeQL and all 12 GitHub checks passed for correction commit `10a2ee9`; no open CodeQL alerts remained on PR #87 at readback. The final adapter commit still needs fresh CI.
