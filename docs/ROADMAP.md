...
Current status: in progress. The first API and schema slice exists with email/password session auth, hash-only single-use email verification and password reset action tokens, SMTP/local auth notification delivery, TOTP MFA challenge flow, hashed recovery codes, MFA-verified admin registration/user management with sanitized audit listing, MFA-verified non-secret provider config and claim-to-role mapping management, hashed scoped API tokens, public search/detail endpoints, MCP token introspection with `skills:read` scope and API-owned session decision audit events, authenticated package intake with strict root-manifest integrity checks, MFA-gated privileged submission, server-side `.zip` archive extraction, scan evidence, S3-compatible object-storage-backed artifact writes and reads, MFA-gated maintainer review approve/publish actions, authorized release metadata and bundle delivery, local `.zip` archive parsing for package validation/submission tooling, a React web app for public registry browsing/detail/export guidance and owner/admin registration/user/provider/audit console workflows, read-only MCP stdio and stateless Streamable HTTP servers for registry discovery/info/install guidance, and a starter CLI for validation, scanning, prompt-based login/logout with API-URL-scoped stored sessions, search, info, whoami, submit, review actions, verified export, local install/list/update/rollback, and API-token management. Queued email delivery, platform keychain CLI token storage, authoritative per-tool MCP audit events, provider login/linking, external identity lifecycle, role editing, signed-url/direct object delivery, platform-specific install adapters, and background scan jobs are still future work.
...
Current status: public browse/search, skill detail, release metadata, export-guidance views, email/password login/logout, MFA challenge completion, current-user refresh, session-aware API calls, and owner/admin console workflows for registration, user status actions, provider metadata/mappings, and audit review exist. Authenticated submit and maintainer review dashboard are still future work.
