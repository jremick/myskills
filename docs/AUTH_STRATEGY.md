...
- Email verification and password reset action tokens are short-lived, single-use, stored only as hashes, and delivered only through the configured notification sink. The current sinks are production SMTP and local-development console delivery.
