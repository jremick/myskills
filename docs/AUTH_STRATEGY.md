...
- Email verification, password reset, registration invitation, and email-change action tokens are short-lived, single-use, stored only as hashes, and delivered only through the configured notification sink. The current sinks are Resend, SMTP, and local-development console delivery.
